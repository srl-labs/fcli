...
flowchart TD
    ...
    Tables --> Srv
    Dev[SR Linux Device] -- gNMI Subscribe STREAM/SAMPLE --> Mem
...
### 3. STREAM / SAMPLE subscriptions
A gNMI `Subscribe` (in STREAM/SAMPLE mode) keeps the node's state tree current. Report getters execute directly against this local tree instead of querying the physical device, rendering tables with zero device round-trips.
...
* **Missing SAMPLE telemetry**: If the route to a node disappears cleanly, TCP connections can silently hang without gRPC keepalives failing. Because all paths are subscribed in SAMPLE mode, the target device must report updates at predictable intervals. Missing expected updates indicates a silent link failure and changes the node status in the sidebar from `up` to unreachable.
